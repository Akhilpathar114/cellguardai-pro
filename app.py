...
def simulate_fleet(n=8):
    ...
    for i in range(n):
        ...
        deg=(100-df["health_score"]).diff().rolling(5).mean().iloc[-1]
        if pd.isna(deg) or deg <= 0:
            deg = 0.2
        m=SCENARIOS[scenario]
        deg*=m["charge"]*m["thermal"]
        fw=(health-55)/deg if deg>0 else np.nan
        fw=max(fw,0) if not pd.isna(fw) else fw
        rows.append({
            "Battery ID":f"BAT-{i+1:02d}",
            "Health":round(health,1),
            "Failure Window (cycles)":safe_int(fw),
...
# ================= SINGLE BATTERY =================
    ...
    health=latest["health_score"]
    deg=(100-df["health_score"]).diff().rolling(10).mean().iloc[-1]
    if pd.isna(deg) or deg <= 0:
        deg = 0.15
    m=SCENARIOS[scenario]
    deg*=m["charge"]*m["thermal"]
    fw=(health-55)/deg if deg>0 else np.nan
    fw=max(fw,0) if not pd.isna(fw) else fw
    low,high=(fw*0.75,fw*1.3) if not pd.isna(fw) else (None,None)
    ...
    c2.metric("Failure Window",f"{safe_int(fw)} cycles")
    c3.metric("Confidence",confidence_level(df))
    c4.metric("Confidence Range",f"{safe_int(low)}–{safe_int(high)}")
    ...
    st.download_button(
        ...
        pd.DataFrame({
            ...
            "Value":[health,safe_int(fw),confidence_level(df)]
        }).to_csv(index=False),
